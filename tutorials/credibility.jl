...
# ╔═╡ 3f97f182-4760-11ed-3a5e-713d2ce6613f
begin
	using Distributions
	using Turing
	using MCMCChains
...
# ╔═╡ 1aeaabdb-b3c0-4928-a8ed-f470d7edc968
md"[https://www.soa.org/globalassets/assets/files/resources/tables-calcs-tools/credibility-methods-life-health-pensions.pdf](https://www.soa.org/globalassets/assets/files/resources/tables-calcs-tools/credibility-methods-life-health-pensions.pdf)"

# ╔═╡ 5f56115e-0666-43be-a188-e4701af3afe1
Logging.disable_logging(Logging.Warn);

# ╔═╡ a9acf62c-b7be-4af4-b256-ac10690b3f7a
θ = [0.51,0.6]

# ╔═╡ d2083158-52b9-4329-880d-de4a7264e75f
@bind fact PlutoUI.Slider(1:100;show_value=true)

# ╔═╡ f8d0f4ca-da8f-45f2-b646-ad162c7136bb
n = [1000,100] .* fact

# ╔═╡ 36fb476e-3499-43f7-a156-b963a0088151
success = rand.(Binomial.(n,θ))

# ╔═╡ 5a8b818a-59fb-42ee-a6f6-b121c0f14d5e
@model function model_binom(success,attempts)
	
	θ_A ~ Beta(1,1)
	θ_B ~ Bernoulli(θ)

	success.A ~ Binomial(attempts.A,θ_A)
	success.B ~ Binomial(attempts.B,θ_B)
end

# ╔═╡ 3d617c42-03b7-49ae-95ca-55dde6d050ce
@model function model_gaussian(success,attempts)
	μ ~ Normal(0.5,0.5)
	σ ~ Exponential(0.25)
	μ_i ~ filldist(Normal(μ,σ),length(attempts))

	for i in 1:length(attempts)
		success[i] ~ Binomial(attempts[i],logistic(μ_i[i]))
	end
end

# ╔═╡ a16ee7b1-53c3-47d8-b633-26a81e7cebe7
mg = model_gaussian(success,n)

# ╔═╡ 6a60cc79-8ca9-48be-947a-2723eab6090d
cg = sample(mg, NUTS(), 500)

# ╔═╡ 9badb1a7-6361-4d08-810c-f19f915f84cd
let
	f = Figure()
	for col in 3:4
		ax = Axis(f[col-2, 1])
		density!(ax,logistic.(cg.value[:,col]))
		xlims!(0.0,1.0)
	end
	f
end

# ╔═╡ ddb53009-03fe-4485-b25c-9e843e179cde
PRECIS(DataFrame(cg))

# ╔═╡ e1455e38-0726-45e8-b4f6-7ba0bfb6205e
success ./ n

# ╔═╡ bcb09819-79de-496a-b4e3-a2e468274a5a
logistic.(mean.(eachslice(cg.value[:,3:4],dims=2)))

# ╔═╡ 6fe2a99a-f5a8-47e1-af16-1dc3b6272951
# ╠═╡ disabled = true
#=╠═╡
chain3 = sample(md, NUTS(), 250)
  ╠═╡ =#

# ╔═╡ db7cc880-dd19-44e0-af99-781be164f6cc
import Makie.plot

# ╔═╡ 9ba9dded-a0c4-4860-aa97-eb2599fc02c5
...
# ╔═╡ 59383615-a0c4-474b-8711-ceecd2427cbc
plot(cg)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "1ba2e1a85a0ecc5962a53a918570e9193ea75915"
...
git-tree-sha1 = "be547b4407b4dce63d357cbc520401e138eeee8e"
...
git-tree-sha1 = "c4193a4b3c69f01918ee048c3851eff24e811957"
