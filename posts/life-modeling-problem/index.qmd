...
```{julia}
#| echo: false
#| label: fig-life-modeling-benchmark-blog
using CairoMakie
using DataFrames
...
let
    # categorical x axis: a "ruler" column for distance equivalents,
    # then the language categories in order of appearance.
    ...
    # distance reference labels stacked on the ruler column
    for (y, lbl) in distances
        text!(ax, 1.0, y; text = lbl, align = (:left, :center),
            color = CF_INK_SOFT, fontsize = 9, offset = (4, 0))
    end

    # one scatter per algorithm — color from the cassette palette
