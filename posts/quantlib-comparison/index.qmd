...
| Modified duration — `duration(Modified(), …)` / `BondFunctions.duration`                 | ~270 ns (AD)          | ~1.25 μs           | ~5×    |
| Convexity — `convexity` / `BondFunctions.convexity`                                      | ~315 ns (nested AD)   | ~1.25 μs           | ~4×    |
...
Second, **AD bundles all 30 pillar partials into one forward pass**, while QL has to run the bond-pricing engine 60 times (30 pillars × 2 sides). The ~1170 μs figure for the per-pillar `ZeroCurve` approach is dominated by rebuilding the curve object on every call. The single-call `KeyRates(tenors)` is one `ForwardDiff.gradient` over a 30-Dual vector — same algorithmic work, no Python boundary in the loop.
...
::: {.callout-note collapse="true"}
## FD vs AD: two routes, one KRD

`duration(KeyRateZero(t), …)` computes each pillar's KRD by finite-differencing the price across a ±10bp triangular hat centered at `t`; `duration(KeyRates(tenors), zrc, cfs)` lays the same hat via `Yield.TenorShift` and walks the analytic gradient through the discount integral in a single ForwardDiff pass. Both return the same KRD vector — the difference is the computation route, not the definition. (As of AU v5.7 the hat shape is decoupled from the base curve's spline, so AD through `Spline.Linear()` and through the default `Spline.MonotoneConvex()` are bit-identical.)
...
krds_ad   = duration(KeyRates(krd_pillars_30), zrc_30, cfs)

maximum(abs.(krds_tent .- krds_ad))   # ≈ FD truncation error
```

The residual is the central-difference truncation, not a different KRD definition. Timing the FD loop on the same setup:
